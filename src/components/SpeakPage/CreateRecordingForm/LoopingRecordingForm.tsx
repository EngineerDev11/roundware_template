...
const LoopingRecordingForm = () => {
    ...
    // useEffect(() => {
    //     // Fetch speakers data when the component mounts
	...
	// 			console.log('Fetched speakers data:', data); // Check the structure of the fetched data
	// 			setSpeakers(data);
	// 		} catch (error) {
	...
	useEffect(() => {
        console.warn('MyComponent mounted');
					...
					<LegalAgreementForm
						...
						onAccept={async () => {
							...
							if (uploadSpeaker) {
								...
								const center = [assetMeta.latitude, assetMeta.longitude]; 
								const radius = 10; 
								...
								const response = await roundware.apiClient.send('/speakers', data, { method: 'POST', contentType: 'multipart/form-data' });
								history.push(`/listen`);
								setSaving(false);
